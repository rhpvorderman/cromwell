...
class CwlJsDecoder extends JsDecoder {
  ...
  def isFile(map: Map[String, AnyRef]): Boolean = {
    map exists {
      case (key, value) => key.toLowerCase == "class" && value.toString.toLowerCase == "file"
    }
  }
  ...
  def isDirectory(map: Map[String, AnyRef]): Boolean = {
    map exists {
      case (key, value) => key.toLowerCase == "class" && value.toString.toLowerCase == "directory"
    }
  }
  ...
  def decodeDirectoryOrFile(value: AnyRef): ErrorOr[FileOrDirectory] = {
    ...
    val partialDecoder: PartialFunction[AnyRef, ErrorOr[FileOrDirectory]] = partialDecodeMapWith { map =>
      if (isFile(map))
        decodeFile(map).map(Coproduct[FileOrDirectory](_))
      else if (isDirectory(map))
        decodeDirectory(map).map(Coproduct[FileOrDirectory](_))
      else
        invalidValue
    }
  ...
  def decodeDirectoryOrFiles(value: AnyRef): ErrorOr[Array[FileOrDirectory]] = {
    val partialDecoder: PartialFunction[AnyRef, ErrorOr[Array[FileOrDirectory]]] = partialDecodeArrayWith {
      array =>
        val result = array.toList.traverse[ErrorOr, FileOrDirectory](decodeDirectoryOrFile)
